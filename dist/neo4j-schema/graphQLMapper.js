...
var mapOutboundRels = function mapOutboundRels(tree, node, config) {
  ...
  return rels
    .map(function(rel) {
      var targetLabels = _lodash['default'].uniq(
        _lodash['default']
          .flatten(
      ...
      if (targetLabels.length > 1) {
        ...
        console.warn(
          'RelID '
            .concat(rel.id, ' for label set ')
            .concat(labels, ' has > 1 target type (')
            .concat(targetLabels, '); skipping')
        );
        return null;
      }
      ...
      var propNavigateToRel = '   '
        .concat(rel.getRelationshipType(), '_rel: [')
        .concat(rel.getGraphQLTypeName(), ']\n');
      return (
    ...
    .map(function(rel) {
      ...
      if (originLabels.length > 1) {
        console.warn(
          'RelID '
            .concat(rel.id, ' for label set ')
            .concat(labels, ' has > 1 origin type (')
            .concat(originLabels, '); skipping')
        );
        return null;
      }
...
var mapNode = function mapNode(tree, node, config) {
  ...
  var propertyDeclarations = propNames.map(function(propName) {
    return '   '
      .concat(propName, ': ')
      .concat(node.getProperty(propName).graphQLType, '\n');
  });
...
var mapRel = function mapRel(tree, rel, config) {
  ...
  var mapUnivalentRel = function mapUnivalentRel(rel) {
    ...
    var typeDeclaration = 'type '
      .concat(graphqlTypeName, ' @relation(name: "')
      .concat(rel.getRelationshipType(), '") {\n'); // It's univalent so this assumption holds:
    ...
    if (!fromNode) {
      throw new Error(
        'No node found in schema tree for univalent rel ' +
          rel.id +
          ' given from labels ' +
          (0, _stringify['default'])(rel.links[0])
      );
    } else if (!toNode) {
      throw new Error(
        'No node found in schema tree for univalent rel ' +
    ...
    var propertyDeclarations = propNames.map(function(propName) {
      return '  '
        .concat(propName, ': ')
        .concat(rel.getProperty(propName).graphQLType, '\n');
    });
    return (
      typeDeclaration +
      fromDecl +
