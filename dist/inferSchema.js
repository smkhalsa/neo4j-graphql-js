...
var generateGraphQLTypeForTreeEntry = function generateGraphQLTypeForTreeEntry(
  ...
  var propertyDeclarations = propNames.map(function(propName) {
    return '   '
      .concat(propName, ': ')
      .concat(entry[propName].graphQLType, '\n');
  });
  ...
  var relDeclarations = _lodash['default'].flatten(
    labels.map(function(label) {
      ...
      return relIds.map(function(relId) {
        ...
        var allTargetLabels = _lodash['default'].uniq(
        ...
        if (allTargetLabels.length > 1) {
          ...
          console.warn(
            'RelID '
              .concat(relId, ' for label ')
              .concat(label, ' has more than one outbound type (')
              .concat(allTargetLabels, '); skipping')
          );
          return null;
        }

        var tag = '@relation(name: "'.concat(
          extractRelationshipType(relId),
          '", direction: "OUT")'
        );
        var targetTypeName = allTargetLabels[0];
        return '   '
          .concat(targetTypeName.toLowerCase(), 's: [')
